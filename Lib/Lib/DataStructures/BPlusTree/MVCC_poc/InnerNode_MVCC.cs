...
{
    class InnerNode_MVCC<TKey, TValue> : Node_MVCC<TKey, TValue>        
        where TKey : IComparable
    {
        ...
        ///  
        /// </summary>
        private Node_MVCC<TKey, TValue>[] _Children;

        #endregion

        #region constructors

        public InnerNode_MVCC(BPlusTree_MVCC<TKey, TValue> myTree) : base(myTree)
        {
            _Children = new Node_MVCC<TKey, TValue>[myTree.MaxChildrenPerNode + 1] ;
        }
        ...
        /// </summary>
        public Node_MVCC<TKey, TValue>[] Children
        {
        ...
        public override bool Insert(TKey myKey, HashSet<TValue> myValue, ref SplitInfo<TKey, TValue> mySplitInfo, IndexSetStrategy myIndexSetStrategy)
        {
            ...
            if (returnVal == SPLITTED)
            {
                ...
                else
                {
                    ...
                    InnerNode_MVCC<TKey, TValue> newNode = new InnerNode_MVCC<TKey, TValue>(_CorrespondingTree);
        ...
        protected override void Transfer(int mySourceStartIndex, int myDestinationStartIndex, int myLength, Node_MVCC<TKey, TValue> myDestinationNode)
        {
            ...
            InnerNode_MVCC<TKey, TValue> destinationNode = myDestinationNode as InnerNode_MVCC<TKey, TValue>;
        ...
        {
            var node = (InnerNode_MVCC<TKey, TValue>)this.MemberwiseClone();

            node._Keys      = (TKey[])this._Keys.Clone();
            node._Children  = (Node_MVCC<TKey, TValue>[])this._Children.Clone();
