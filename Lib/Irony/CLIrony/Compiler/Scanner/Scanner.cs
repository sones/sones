...
namespace sones.Lib.Frameworks.CLIrony.Compiler {
  ...
  public class Scanner  {
    ...
    public List<List<Token>> BeginNonDetermisticScan()
    {
        List<List<Token>> _tokenList = new List<List<Token>>();
        List<Token> _currentTokenList = new List<Token>();
        int Counter = 0;

        while (true)
        {
            //get next Token as List
            _currentTokenList = ReadTokenList();

            _tokenList.Add(_currentTokenList);

            if (_tokenList[Counter][0].Term.Equals(Grammar.Eof))
            {
                break;
            }
            //left out event-handling
            /*
            if (!_currentTokenList.Count.Equals(0))
            {
                OnTokenCreated(_currentTokenList);
            }
            */
            Counter++;
        }
    ...
    private List<Token> ReadTokenList()
    {
        List<Token> _currentTokenList = new List<Token>();
        ...
        resultList = MatchTerminalsImproved(terms);
        _currentTokenList.AddRange(resultList);

        //If no token in List, try FallbackTerminals
        if (resultList.Count.Equals(0) && Data.FallbackTerminals.Count > 0)
        {
            resultList = MatchTerminalsImproved(Data.FallbackTerminals);
            _currentTokenList.AddRange(resultList);
        }

        //TryMatch always returns null, so it is left out
        ...
        List<Token> _ToBeDeletedTokenList = new List<Token>();
        List<Token> _ToBeAddedTokenList = new List<Token>();
        foreach (Token aToken in _currentTokenList)
        {
            if (aToken.IsMultiToken())
            {
                foreach (Token tkn in aToken.ChildNodes)
                    _bufferedTokens.Add(tkn);

                _ToBeDeletedTokenList.Add(aToken);
                _ToBeAddedTokenList.Add(_bufferedTokens[0]);
                
                _bufferedTokens.RemoveAt(0);
            }
        }
        foreach (Token ToBeRemovedToken in _ToBeDeletedTokenList)
        {
            _currentTokenList.Remove(ToBeRemovedToken);
        }
        _currentTokenList.AddRange(_ToBeAddedTokenList);

        //If we have normal tokens then return it
        if (_currentTokenList.Count > 0)
        {
            _source.Position = _source.TokenStart.Position + _currentTokenList[0].Length;
            return _currentTokenList;
        }
        else
        {
    ...
    private List<Token> MatchTerminalsImproved(TerminalList terminals)
    {
        List<Token> _aTokenList = new List<Token>();
        Token result = null;
        
        foreach (Terminal term in terminals)
        {
            if (_aTokenList.Count > 0)
            {
                if (_aTokenList[0].Terminal.Priority < term.Priority)
                {
                    //we have a term with higher priority, so the token-list has to be cleared
                    _aTokenList.Clear();

                }//new term has higher priority ?
                else
                {
                    if (_aTokenList[0].Terminal.Priority > term.Priority)
                    {
                        //we can stop iterating all the terminals, because they are ordered by 
                        //priority. terms with lower priority are not interesting.
                        break;
                    }
                }//new term has no higher priority !
            }
            ...
            //Take this token as result only if we don't have anything yet
            if (token != null && !token.IsError())
            {
                result = token;
                _aTokenList.Add(result);
            }
