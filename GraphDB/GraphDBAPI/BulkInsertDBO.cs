...
namespace GraphDBAPI
{
    public class BulkInsertDBO
    {
        ...
        /// <returns></returns>
        public BulkInsertDBO AddAttribute(String myAttribute, AObject myValue)
        {
        ...
        /// <param name="myValue">The value</param>
        /// <returns></returns>
        public BulkInsertDBO AddAttribute(TypeAttribute myAttribute, AObject myValue)
        {
        ...
        public BulkInsertDBO AddEdge(TypeAttribute myEdgeAttribute, ObjectUUID myObjectReference, params ADBBaseObject[] myEdgeParams)
        {
            ...
            if (!myEdgeAttribute.GetDBType(_DBContext.DBTypeManager).IsUserDefined)
                throw new GraphDBException(new Error_InvalidEdgeType(typeof(ASetReferenceEdgeType), typeof(ASingleReferenceEdgeType)));

            #endregion

            if (!_DBObjectStream.HasAttribute(myEdgeAttribute.UUID, myEdgeAttribute.GetRelatedType(_DBContext.DBTypeManager), null))
            ...
            if (myEdgeAttribute.KindOfType == KindsOfType.SetOfReferences)
                (_DBObjectStream.GetAttribute(myEdgeAttribute.UUID) as ASetReferenceEdgeType).Add(myObjectReference, myEdgeParams);           
            else
                (_DBObjectStream.GetAttribute(myEdgeAttribute.UUID) as ASingleReferenceEdgeType).Set(myObjectReference, myEdgeParams);
        ...
        public BulkInsertDBO AddEdge(TypeAttribute myEdgeAttribute, IEnumerable<ObjectUUID> myObjectReferences, params ADBBaseObject[] myEdgeParams)
        {
            ...
            if (myEdgeAttribute.KindOfType == KindsOfType.SetOfReferences)
                (_DBObjectStream.GetAttribute(myEdgeAttribute.UUID) as ASetReferenceEdgeType).Add(myObjectReferences, myEdgeParams);
