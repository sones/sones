...
namespace GraphDBAPI
{
    ...
    public class BulkInsert : IDisposable
    {
        ...
        public BulkInsert(IGraphDBSession myGraphDBSession, IGraphFSSession myGraphFSSession, String myType)
        {
            ...
            _DBTransaction = _GraphDBSession.BeginTransaction(myLongRunning: true, myIsolationLevel: IsolationLevel.Serializable);
            _Type = _DBTransaction.GetDBContext().DBTypeManager.GetTypeByName(myType);
        }
        ...
        public BulkInsertDBO Insert(ObjectUUID myObjectUUID)
        {
            ...
            UnitTestHelper.SetPrivateProperty("ObjectUUID", DBObjectStream, myObjectUUID);
            DBObjectStream.ObjectLocation = new ObjectLocation(DBObjectStream.ObjectLocation.Path, DBObjectStream.ObjectUUID.ToString());

            return new BulkInsertDBO(_DBTransaction.GetDBContext(), _Type, DBObjectStream, this);
