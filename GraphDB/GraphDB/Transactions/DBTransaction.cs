...
namespace sones.GraphDB.Transactions
{
    ...
    public class DBTransaction : ATransaction
    {
        ...
        /// </summary>
        private DBContext _SessionDBContext;
        private DBContext _TransactionalDBContext;
            ...
            : base(myDistributed, myLongRunning, myIsolationLevel, myName, timestamp)
        {
            ...
            _TransactionalDBContext = new DBContext(GetDBContext());
            _Session = session;
        }

        public DBTransaction(DBContext dbContext, SessionUUID session, FSTransaction fsTransaction)
            :base(fsTransaction)
        {
            _FSTransaction = fsTransaction;

            _SessionDBContext = dbContext;
            _TransactionalDBContext = new DBContext(dbContext);
            _Session = session;
        }
            ...
            IsolationLevel myIsolationLevel = IsolationLevel.Serializable, String myName = "", DateTime? timestamp = null)
        {
            ...
            else
            {
                ...
                dbTransaction.Push(new Warnings.Warning_NoFSTransaction());
            }
