...
namespace sones.GraphDB.Managers.Select
{
	...
	public class SelectResultManager
	{
        ...
        /// <summary>
        /// These special type attribute will be always added to the result
        /// </summary>
        List<TypeAttribute> _DefaultSpecialAttributes = new List<GraphDB.TypeManagement.TypeAttribute>();
		...
		/// <returns></returns>
		private IEnumerable<Vertex> ExamineDBO_Aggregates(IEnumerable<Exceptional<DBObjectStream>> myDBOs, List<SelectionElementAggregate> myAggregates, List<SelectionElement> mySelections, GraphDBType myReferencedDBType, Boolean myUsingGraph, Int64 myDepth)
		{
			...
			if (mySelections.CountIsGreater(0))
			{
				...
				foreach (var group in groupedDBOs)
				{
					...
					foreach (var aggr in myAggregates)
					{
                        ...
                        if (aggrResult.Value.Value != null)
                        {
                            //aggregatedAttributes.Add(aggr.Alias, aggrResult.Value.Value.GetReadoutValue());
                            aggregatedAttributes.Add(aggr.Alias, ResolveAggregateResult(aggrResult.Value, myDepth));
                        }
			...
			else
			{
				...
				if (!myUsingGraph && myAggregates.All(a => a.IndexAggregate != null))
				{
					...
					foreach (var aggr in myAggregates)
					{
                        ...
                        //aggregatedAttributes.Add(aggr.Alias, idxAggrResult.Value.Value.GetReadoutValue());
                        if (idxAggrResult.Value.Value != null)
                        {
                            aggregatedAttributes.Add(aggr.Alias, ResolveAggregateResult(idxAggrResult.Value, myDepth));
                        }
					}
        ...
        private Object ResolveAggregateResult(FuncParameter aggrResult, Int64 myDepth)
        {
            ...
            if (aggrResult.Value is IReferenceEdge)
            {
            ...
            else
            {
                ...
                return aggrResult.Value.GetReadoutValue();
		...
		/// <param name="myUsingGraph"></param>
		/// <returns></returns>
        private Vertex LoadAndResolveVertex(ObjectUUID myObjectUUID, GraphDBType myTypeOfAttribute, Int64 myDepth, EdgeList myLevelKey, String myReference, Boolean myUsingGraph, bool mySelectAllAttributes = false)
		{
			...
			return new Vertex(GetAllSelectedAttributesFromVertex(dbStream.Value, myTypeOfAttribute, myDepth, myLevelKey, myReference, myUsingGraph, mySelectAllAttributes));
        ...
        private void AddAttributesByDBO(ref Dictionary<String, Object> myAttributes, GraphDBType myType, DBObjectStream myDBObject, Int64 myDepth, EdgeList myEdgeList, String myReference, Boolean myUsingGraph, TypesOfSelect mySelType, TypeUUID myTypeID = null)
        {
            ...
            if (myAttributes.Count > 0) // Add only default attributes if at least one other attribute were selected
            {
                foreach (var specialAttr in GetDefaultSpecialAttributes())
                {
                    if (!myAttributes.ContainsKey(specialAttr.Name))
                    {
                        var result = (specialAttr as ASpecialTypeAttribute).ExtractValue(myDBObject, myType, _DBContext);
        ...
        private IEnumerable<TypeAttribute> GetDefaultSpecialAttributes()
        {

            return _DefaultSpecialAttributes;
		...
		/// <param name="myUsingGraph"></param>
		/// <returns></returns>
		public Dictionary<string, object> GetAllSelectedAttributesFromVertex(DBObjectStream myDBObject, GraphDBType myDBType, Int64 myDepth, EdgeList myLevelKey, String myReference, Boolean myUsingGraph, Boolean selectAllAttributes = false)
		{
				...
				foreach (var attrSel in attributeSelections)
				{
					...
					if (attrSel is SelectionElementFunction)
					{
                        ...
                        if (myUsingGraph)
                        {
                            ...
                            myUsingGraph = _ExpressionGraph.IsGraphRelevant(new LevelKey((myLevelKey + key).Edges, _DBContext.DBTypeManager), myDBObject);
