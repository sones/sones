            ...
            foreach (var keyValPair in _Attributes)
            {
                ...
                foreach (var idChain in keyValPair.Value)
                {
                    ...
                    foreach (var pSetting in mySettingValues)
                    {
                        ...
                        {
                            _DBContext.DBSettingsManager.SetSetting(pSetting.Key.ToUpper(), GetValueForSetting(_DBContext.DBSettingsManager.AllSettingsByName[pSetting.Key.ToUpper()], pSetting.Value), _DBContext, TypesSettingScope.ATTRIBUTE, type, idChain.LastAttribute);
                        }
                        else
                        {
                            return new Exceptional<List<SelectionResultSet>>(new Error_SettingDoesNotExist(pSetting.Key));
