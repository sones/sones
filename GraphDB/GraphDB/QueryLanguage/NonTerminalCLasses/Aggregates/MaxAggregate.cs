...
/* <id Name=”PandoraDB – MaxAggregate” />
...
namespace sones.GraphDB.QueryLanguage.NonTerminalCLasses.Aggregates
{
    public class MaxAggregate : ABaseAggregate
    {
        ...
        public override Exceptional<object> Aggregate(AttributeIndex attributeIndex, GraphDBType graphDBType, DBContext dbContext, DBObjectCache myDBObjectCache, SessionSettings mySessionToken)
        {
            ...
            return new Exceptional<Object>((ObjectUUID)idxRef.Value.Keys().Max().IndexKeyValues[0].Value);

            //return new Exceptional<object>(SpecialTypeAttribute_UUID.ConvertFromUUID((ObjectUUID)idxRef.Value.Keys().Max().IndexKeyValues[0].Value, settingEncoding));
