...
{
    public sealed class UpdateEdgesNode : AStatement, IAstNodeInit
    {
        #region Data
        ...
        public void Init(String myTypeName,
                            IEnumerable<AAttributeAssignOrUpdateOrRemove> myAttributeAssignList, 
                            IEnumerable<IVertex> myVertexIDs)
        {
        ...
        public void Init(ParsingContext context, ParseTreeNode parseNode)
        {
            ...
            _TypeName = ((AstNode)parseNode.ChildNodes[3].AstNode).AsString;
            ...
            if (parseNode.ChildNodes[4].ChildNodes != null && parseNode.ChildNodes[5].ChildNodes.Count != 0)
            {
                                                    ...
                                                    Int64 myTransactionToken)
        {
            ...
            if (_vertexIDs == null)
            {
                if (_WhereExpression != null)
                {
                    ...
                    //calculate
                    var expressionGraph = 
                        _WhereExpression.Calculon(myPluginManager, 
                                                    myGraphDB, 
                                                    mySecurityToken, 
                                                    myTransactionToken, 
                                                    new CommonUsageGraph(myGraphDB, 
                    ...
                    //extract
                    toBeupdatedVertices = 
                        expressionGraph.Select(new LevelKey(vertexType.ID, 
                                                            myGraphDB, 
                                                            mySecurityToken, 
                                                            myTransactionToken), 
                                                null, true);
            ...
            var result = new RequestUpdate(new RequestGetVertices(vertexType.ID, 
                                                                    toBeupdatedVertices
                                                                        .Select(_ => _.VertexID), false));
                                    ...
                                    ref RequestUpdate result)
        {
            ...
            {
                ProcessAttributeAssignOrUpdateValue((AttributeAssignOrUpdateValue)aUpdate, 
                                                    ref result);
            }
                ...
                if (((TupleDefinition)attributeRemoveList.TupleDefinition).All(_ => _.Value is ValueDefinition))
                {
                    ...
                    if (vertexType.HasProperty(attributeRemoveList.AttributeIDChain.ContentString))
                    {
                        myRequestedType = ((IPropertyDefinition)vertexType
                                            .GetAttributeDefinition(attributeRemoveList.AttributeIDChain.ContentString))
                                            .BaseType;
                    }
                    else
                    {
                        ...
                        if (aTupleElement.Value is BinaryExpressionDefinition)
                        {
                            ...
                            if (!vertexType.HasAttribute(attributeRemoveList.AttributeName))
                            {
                                throw new InvalidVertexAttributeException(String.Format("The vertex type {0} has no attribute named {1}.", 
                                                                            vertexType.Name, 
                                                                            attributeRemoveList.AttributeName));
                            }
                            ...
                            if (toBeRemovedEdges.TryGetValue(attributeRemoveList.AttributeName, out outValue))
                            {
                                ...
                                {
                                    outValue.AddEdge(new EdgePredefinition()
                                                            .AddVertexID(aVertex.VertexTypeID, aVertex.VertexID));
                                }
                            }
                            else
                            {
            ...
            switch (attributeAssignOrUpdateList.CollectionDefinition.CollectionType)
            {
                case CollectionType.Set:
                    ...
                    if (((TupleDefinition)attributeAssignOrUpdateList.CollectionDefinition.TupleDefinition).All(_ => _.Value is ValueDefinition))
                    {
                        ...
                        {
                            myRequestedType = ((IPropertyDefinition)vertexType
                                                .GetAttributeDefinition(attributeAssignOrUpdateList.AttributeIDChain.ContentString))
                                                .BaseType;
                        }
                        else
                        {
                        ...
                        {
                            setWrapper
                                .Add(((ValueDefinition)aTupleElement.Value)
                                .Value
                                .ConvertToIComparable(myRequestedType));
                        }
