...
namespace sones.GraphFS.Connectors.GraphFSCLI
{
    ...
    public class StringLiteral_IPandoraFS : AGraphFSCLIAutocompletions
    {
        ...
        public override List<String> Complete(ref object PFSObject, ref object PDBObject, ref String CurrentPath, string CurrentStringLiteral)
        {
            ...
            IGraphFSSession     _IGraphFSSession    = (IGraphFSSession)PFSObject;
            ...
            if (CurrentStringLiteral.Length.Equals(0))
            {
                try
                {
                    _DirectoryElements = _IGraphFSSession.GetDirectoryListing(new ObjectLocation(CurrentPath)).Value;
                }
                catch
                {
                    //do nothing
            ...
            else
            {
                ...
                if (CurrentStringLiteral.StartsWith(FSPathConstants.PathDelimiter))
                {
                    ...
                    try
                    {
                        directoryExists = _IGraphFSSession.isIDirectoryObject(new ObjectLocation(SimplifyObjectLocation(PrefixDir))).Value == Trinary.TRUE;
                    }
                    catch
                    {
                        //do nothing
                    ...
                    if (directoryExists)
                    {
                        ...
                        try
                        {
                            _DirectoryElements = _IGraphFSSession.GetDirectoryListing(new ObjectLocation(SimplifyObjectLocation(PrefixDir))).Value;
                        }
                        catch
                        {
                            //do nothing
                ...
                else
                {
                    ...
                    else
                    {
                        try
                        {
                            _DirectoryElements = _IGraphFSSession.GetDirectoryListing(new ObjectLocation(SimplifyObjectLocation(CurrentPath + FSPathConstants.PathDelimiter + PrefixDir))).Value;
                        }
                        catch
                        {
                            //do nothing
